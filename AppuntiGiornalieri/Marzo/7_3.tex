...
\label{eqn:ev-temp}
\end{align}
Ci interessa quantificare quanto $\ket{\psi(t)}$ sia \q{vicino} allo stato iniziale $\ket{\psi_0}$. Consideriamo allora l'\textit{ampiezza} della funzione d'onda che è rimasta \textit{parallela} a $\ket{\psi_0}$, e che è data dal prodotto scalare: \marginpar{Survival amplitude}
\begin{align}
\mathcal{A}(t) = \braket{\psi_0|\psi(t)} = \bra{\psi_0}e^{-iHt}\ket{\psi_0}
\label{eqn:surv-amplitude}
\end{align}
Prendendo il modulo quadro otteniamo la \textit{probabilità} $p(t)$ che lo stato al tempo $t$ sia ancora pari a quello iniziale, detta \textbf{survival probability}\index{Effetto Zenone!Survival probability}: \marginpar{Survival probability}
\begin{align}
...
Abbiamo allora ricavato che i valor medi dei \q{termini misti} sono nulli. Inoltre, dato che $\ket{\psi_0}$ è autostato di $H_0$ vale (per definizione di autostato):
\begin{align*}
...
Come vedremo nei prossimi paragrafi, questa idea di \textit{interazione} sorge esaminando nel dettaglio i \q{retroscena} delle proiezioni di Von Neumann.
...
\input{Tikz/7_3/evoluzioneUnitaria.tex}
...
Per trattare l'evoluzione non unitaria introduciamo allora \textit{ad hoc} una \q{Hamiltoniana modificata} data da:
\begin{align}
...
In generale, l'ampiezza delle oscillazioni di probabilità è $<1$. Solo nel caso risonante, con $\omega = \omega_0$, si ottengono \q{oscillazioni complete} - ossia tali che, dopo $\pi/\Omega$, si abbia probabilità di transizione a $\ket{1}$ (partendo da $\ket{0})$ pari a $1$ (certezza). In tal caso, inoltre $\Omega = \omega_1$ (figura \ref{fig:plot-rabi}).
